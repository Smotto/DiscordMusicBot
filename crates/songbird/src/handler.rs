...
impl Call {
    ...
    pub fn update_server(&mut self, endpoint: String, token: String) {
        self.apply_server_change(|progress| progress.apply_server_update(endpoint, token));
    }

    /// Apply a voice-server token when Discord omits the endpoint (region migration).
    pub fn update_server_token(&mut self, token: String) {
        self.apply_server_change(|progress| progress.apply_server_token(token));
    }

    /// Apply a voice-server credential change without rebuilding the UDP session when
    /// the call is already connected.
    fn apply_server_change(&mut self, apply: impl FnOnce(&mut ConnectionProgress) -> bool) {
        let was_in_progress = self
            .connection
            .as_ref()
            .map(|(progress, _)| progress.in_progress())
            .unwrap_or(false);

        let changed = if let Some((ref mut progress, _)) = self.connection.as_mut() {
            apply(progress)
        } else {
            false
        };
        ...
        if was_in_progress {
            ...
            #[cfg(feature = "driver")]
            self.hot_update_driver_info();
        }
    }

    #[cfg(feature = "driver")]
    fn hot_update_driver_info(&mut self) {
