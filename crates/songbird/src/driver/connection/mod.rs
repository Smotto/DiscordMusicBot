...
async fn init_cipher(
    ...
    loop {
        ...
        match value {
            GatewayEvent::SessionDescription(desc) => {
                ...
                let dave_session =
                    if let Some(version) = NonZeroU16::new(desc.dave_protocol_version) {
                        info!("DAVE: Creating DaveSession version={}", version);
                        ...
                        info!("DAVE: DaveMlsKeyPackage sent successfully");
                        Some(session)
                    } else {
                        None
                    };
            ...
            other => {
                // Discord sends user-specific packets (e.g. ClientsConnect) during this step.
