                ...
                if let Some((rtp_body_start, rtp_body_tail, decrypted)) = packet_data {
                            ...
                            == DAVE_MAGIC_MARKER
                    {
                        ...
                        let Some(user_id) = self.ssrc_signalling.ssrc_user_map.get(&ssrc) else {
                            tracing::debug!("DAVE: SSRC={} not in ssrc_user_map, dropping", ssrc);
                            return;
                        };
                        let Some(ref mut dave_session) = *self.dave_session.write().unwrap() else {
                            tracing::debug!("DAVE: no dave_session, dropping SSRC={}", ssrc);
                            return;
                        };

                        if !dave_session.is_ready() {
                            tracing::debug!("DAVE: session not ready for SSRC={}", ssrc);
                            return;
                        }
                        ...
                        match result {
                            Ok(decrypted_body) => {
                                tracing::debug!("DAVE: decrypted SSRC={} body {}->{} bytes", ssrc, body_length, decrypted_body.len());
                                packet_data = Some((
                                    rtp_body_start,
                            ...
                            | Err(davey::errors::DecryptError::DecryptionFailed(
                                ...
                                tracing::debug!("DAVE: no decryptor for user SSRC={}", ssrc);
