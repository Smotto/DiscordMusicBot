    ...
    while let Ok(msg) = rx.recv_async().await {
        match msg {
            ...
            CoreMessage::SignalWsClosure(ws_idx, ws_info, mut reason) => {
                info!("SignalWsClosure: ws_idx={}, attempt_idx={}, reason={:?}", ws_idx, attempt_idx, reason);
                ...
                let conn = if ws_idx == attempt_idx {
                    info!("SignalWsClosure: idx matches, dropping connection");
                    drop(interconnect.mixer.send(MixerMessage::DropConn));
                    drop(interconnect.mixer.send(MixerMessage::RebuildEncoder));
                    connection.take()
                } else {
                    info!("SignalWsClosure: idx mismatch (ws_idx={} != attempt_idx={}), ignoring", ws_idx, attempt_idx);
                    reason = None;
                    None
                };
                ...
                if conn.is_some() {
                    info!("SignalWsClosure: firing DriverDisconnect event");
                    drop(interconnect.events.send(EventMessage::FireCoreEvent(
                        CoreContext::DriverDisconnect(InternalDisconnect {
                            kind: DisconnectKind::Runtime,
                            reason,
            ...
            CoreMessage::UpdateConnectionInfo(info) => {
                if let Some(conn) = connection.as_mut() {
                    conn.info = info.clone();
                    let _ = conn.ws.send(WsMessage::UpdateInfo(info));
                }
            },
...
impl ConnectionRetryData {
    ...
    async fn attempt(
        ...
        info!("ConnectionRetryData::attempt idx={} attempts={}", self.idx, self.attempts);
        match Connection::new(self.info.clone(), interconnect, config, self.idx).await {
            Ok(connection) => {
                info!("ConnectionRetryData::attempt SUCCESS idx={}", self.idx);
                match self.flavour {
            ...
            Err(why) => {
                info!("ConnectionRetryData::attempt FAILED idx={} error={}", self.idx, why);
