...
impl Driver {
    ...
    /// Refresh voice gateway credentials on an existing connection without
    /// tearing down the UDP/DAVE session.
    pub(crate) fn update_connection_info(&mut self, info: ConnectionInfo) {
