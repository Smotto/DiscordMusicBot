...
impl ParkedMixer {
    ...
    pub fn handle_message(&mut self, msg: MixerMessage) -> Result<bool, ()> {
        match msg {
            ...
            MixerMessage::Ws(ws) => {
                self.mixer.ws = ws;
                // Re-assert speaking after a voice websocket resume so Discord keeps
                // accepting RTP. The old behaviour sent "not speaking" on every WS
                // swap, which caused audible dropouts mid-track.
                if self
                    .mixer
                    .tracks
                    .iter()
