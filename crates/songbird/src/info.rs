...
impl ConnectionProgress {
    ...
    pub(crate) fn apply_server_token(&mut self, token: String) -> bool {
        match self {
            Self::Complete(c) => {
                let should_reconn = c.token != token;
                c.token = token;
                should_reconn
            },
            Self::Incomplete(i) => i
                .apply_server_token(token)
                .map(|info| {
                    *self = Self::Complete(info);
                })
                .is_some(),
...
impl Partial {
    ...
    fn apply_server_token(&mut self, token: String) -> Option<ConnectionInfo> {
        self.token = Some(token);
        self.finalise()
    }
}
